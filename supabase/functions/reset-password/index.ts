...
serve(async (req) => {
  ...
  try {
    ...
    const notificationResponse = await fetch('https://api.notificationapi.com/aktivlogg/sender', {
      method: 'POST',
      headers: {
    ...
    if (!notificationResponse.ok) {
      ...
      throw new Error(`Failed to send email: ${notificationResponse.status} ${errorText}`);
    }
