-- Fix infinite recursion in RLS policies for upload functionality (v2)
...
-- 5. Drop ALL existing storage policies to avoid conflicts
...
DROP POLICY IF EXISTS "Users can view their own target images" ON storage.objects;
DROP POLICY IF EXISTS "Users can upload their own target images" ON storage.objects;
DROP POLICY IF EXISTS "Users can update their own target images" ON storage.objects;

-- Create simplified storage policies using auth.uid() to avoid RLS cascades
...
CREATE POLICY "Logos publicly viewable" 
ON storage.objects 
FOR SELECT 
...
CREATE POLICY "Users upload own targets" 
ON storage.objects 
FOR INSERT 
WITH CHECK (
  bucket_id = 'target-images' 
